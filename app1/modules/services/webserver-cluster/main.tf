...
resource "aws_security_group" "alb_sg" {
  name = "${var.cluster_name}_allow ALB"
  tags = var.tags
  ingress {
    cidr_blocks = [ var.public_cidr]
    description = "allow to port 80"
    from_port = 80
    to_port = 80
    protocol = "tcp"        
  } 
...
resource "aws_key_pair" "tf_deployer" {
  key_name   = "${var.cluster_name}_tf_deployer"
